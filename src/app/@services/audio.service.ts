...
export class AudioService {
  ...
  private calculateTime = (evt) => {
    ...
    this.setTimeDuration(duration);
    this.setPercentElapsed(duration, currentTime);
  ...
  private setPercentElapsed(duration: number, currentTime: number): void {
    this.percentElapsed.next(Math.floor((100 / duration) * currentTime) || 0);
  }
   ...
   * @param position
   */
  private setPercentLoaded(position): void {
    this.percentLoaded.next(parseInt(position, 10) || 0);
  }
  ...
  private setTimeDuration(duration): void {
    let seconds = Math.floor(duration % 60),
      displaySecs = seconds < 10 ? '0' + seconds : seconds,
      minutes = Math.floor((duration / 60) % 60),
      displayMins = minutes < 10 ? '0' + minutes : minutes;

    this.timeDuration.next(displayMins + ':' + displaySecs);
  }

  public getTimeDuration(): Observable<string> {
    return this.timeDuration.asObservable();
  }
  ...
  // write function to mute audio
  public muteAudio(isMuted): void {
